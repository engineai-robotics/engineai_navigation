...
void SuperLIOReLoc::process()
{
  ...
  if (flg_get_init_guess_ && kf_->init_) {
    kf_->init_ = false;
    state_fn_ = &SuperLIOReLoc::stateWaitKFInit;
    if (!init_obs_data_) {
      init_obs_data_.reset(new PointCloudType());
    ...
    LOG(INFO) << YELLOW
              << " ---> RViz relocalization requested, re-running ICP"
              << RESET;
  }
  (this->*state_fn_)();
}
...
bool SuperLIOReLoc::kf_init(){
  ...
  if(flg_get_init_guess_){
    ...
    received_rviz_pose_ = true;
    LOG(INFO) << YELLOW
              << " ---> Accumulating lidar/IMU for ICP, keep the robot still"
              << RESET;
    return false;
  }

  if (!received_rviz_pose_) {
    LOG_FIRST_N(INFO, 1) << YELLOW
              << " ---> Waiting for RViz 2D Pose Estimate on /initialpose"
              << RESET;
    return false;
  }
  ...
  // if (icp.hasConverged() == false)
  {
    ...
    mean_gyro = V3::Zero();
    mean_acce = V3::Zero();
    LOG(INFO) << RED << " ---> Global ICP Converged Fail! FitnessScore: " << icp.getFitnessScore()
              << ". Give a new 2D Pose Estimate in RViz." << RESET;
    received_rviz_pose_ = false;
    return false;
  } else{
