...
from interface_protocol.msg import BodyVelCmd, GamepadKeys
...
class RlNavigationNode(Node):
    ...
    def __init__(self):
        ...
        self.body_vel_cmd_topic = self.declare_parameter(
            "body_vel_cmd_topic", "/motion/body_vel_cmd"
        ).value
        self.body_vel_cmd_hz = float(
            self.declare_parameter("body_vel_cmd_hz", 50.0).value
        )
        self.body_vel_scale = np.asarray(
            self.declare_parameter(
                "body_vel_scale", [0.7, 0.4, 0.7]
            ).value,
            dtype=np.float64,
        )
        ...
        self.gamepad_type = str(
            self.declare_parameter("gamepad_type", "gamepad_keys").value
        )
        self.gamepad_topic = self.declare_parameter(
            "gamepad_topic", "/hardware/gamepad_keys"
        ).value
        self.mode_button = int(
            self.declare_parameter("mode_button", GamepadKeys.BACK).value
        )
        self.disable_button = int(
            self.declare_parameter("disable_button", GamepadKeys.X).value
        )
        self.enable_button = int(
            self.declare_parameter("enable_button", GamepadKeys.Y).value
        )
        ...
        if (
            ...
            or self.body_vel_scale.shape != (3,)
            or not np.all(np.isfinite(self.body_vel_scale))
            or np.any(self.body_vel_scale < 0.0)
            ...
            or self.gamepad_type not in ("gamepad_keys", "joy")
            or self.joystick_cmd_vel_topic == self.cmd_vel_topic
            or self.body_vel_cmd_hz < 0.0
        ...
        self.body_vel_publisher = None
        if self.body_vel_cmd_topic:
            self.body_vel_publisher = self.create_publisher(
                BodyVelCmd, self.body_vel_cmd_topic, 10
            )
        ...
        if self.gamepad_type == "gamepad_keys":
            self.gamepad_subscription = self.create_subscription(
                GamepadKeys,
                self.gamepad_topic,
                self._on_gamepad_keys,
                qos_profile_sensor_data,
            )
        else:
            self.gamepad_subscription = self.create_subscription(
        ...
        if self.body_vel_publisher is not None and self.body_vel_cmd_hz > 0.0:
            self.body_vel_timer = self.create_timer(
                1.0 / self.body_vel_cmd_hz, self._on_body_vel_hold
            )
        ...
        self.get_logger().info(
            ...
            f"body_vel={self.body_vel_cmd_topic or 'disabled'}, "
            f"mode={'navigation' if self.policy_enabled else 'joystick'}"
        )
        self.get_logger().info(
            f"Gamepad gate on {self.gamepad_topic} ({self.gamepad_type}): "
            f"BACK/MODE({self.mode_button}) + X({self.disable_button}) "
            f"disables, BACK/MODE({self.mode_button}) + "
    ...
    @staticmethod
    def _button_pressed(states, index: int) -> bool:
        return index < len(states) and int(states[index]) != 0
    ...
    def _set_policy_enabled(self, enabled: bool, source: str) -> bool:
        ...
        if enabled:
            ...
            self.get_logger().info(
                f"ROS velocity commands stopped by {source}; "
                "native gamepad control"
            )
        return True
    ...
    def _on_set_policy_enabled(self, request, response):
        ...
        if request.data:
            ...
            response.message = (
                "RL navigation disabled"
                if changed
                else "RL navigation already disabled"
            )
        return response

    def _on_gamepad_keys(self, message: GamepadKeys) -> None:
        """Gate policy velocity using GamepadKeys BACK+X / BACK+Y."""
        if not message.hardware_connected:
            return
        self._update_policy_gate(message.digital_states)

    def _on_joy(self, message: Joy) -> None:
        """Gate policy velocity using sensor_msgs/Joy MODE+X / MODE+Y."""
        self._update_policy_gate(message.buttons)

    def _update_policy_gate(self, states) -> None:
        """Gate policy velocity output using MODE+X and MODE+Y."""
        mode_pressed = self._button_pressed(states, self.mode_button)
        disable_pressed = mode_pressed and self._button_pressed(
            states, self.disable_button
        )
        enable_pressed = mode_pressed and self._button_pressed(
            states, self.enable_button
        )
    ...
    def _publish_body_vel(self, command: np.ndarray) -> None:
        """Publish physical body velocity for the hardware motion stack."""
        if self.body_vel_publisher is None:
            return
        scaled = np.asarray(command, dtype=np.float64) * self.body_vel_scale
        message = BodyVelCmd()
        message.header.stamp = self.get_clock().now().to_msg()
        message.linear_velocity = [float(scaled[0]), float(scaled[1])]
        message.yaw_velocity = float(scaled[2])
        self.body_vel_publisher.publish(message)

    def _on_body_vel_hold(self) -> None:
        """Keep /motion/body_vel_cmd alive only while navigation is enabled."""
        if self.policy_enabled:
            self._publish_body_vel(self.last_command)
    ...
    def _publish(self, action: np.ndarray) -> None:
        ...
        self._publish_body_vel(command)
